...
上記40設定のCI回帰マトリクスには未追加(C++参照実装がtype 4/5を持たないため機械的な
突合せができず、`matrix/`フィクスチャ化を見送っている)。代わりに個別に、Pythonで
学習した実モデルから`.treg`を書き出し→JSでロード・予測→`train_bridge.py`/
`predict_template.py`側の予測値と突き合わせる形で検証:
...
- `matrix/` — 40通りの`.treg`フィクスチャ(`_manifest.json`に一覧)
...
```bash
npm install     # jsdomのみ(run_html_matrix_test.js用)。他は依存ゼロ
npm test        # run_parity.js + run_matrix_test.js + run_html_matrix_test.js を一括実行
```
